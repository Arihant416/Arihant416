    ...
    <div className={ `fixed top-0 left-0 right-0 z-[100] flex justify-center px-4 sm:px-8 md:px-12 pt-3 pb-3 transition-all duration-300 ${scrolled
      ...
      >
        ...
        <div className="flex-1 flex justify-start">
          <a
            href="#top"
            className="inline-flex min-h-11 min-w-11 items-center justify-center font-mono text-xs font-semibold tracking-tighter !text-text hover:!text-accent focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent rounded-full px-3 py-1 border border-transparent hover:border-border transition-all duration-300"
          >
            AJ
          </a>
        </div>
        ...
        <div className="hidden sm:flex items-center bg-card border border-border/60 rounded-full p-1 shadow-sm relative">
          { NAV.map(({ label, href }, idx) => (
            <a
              key={ href }
              href={ href }
              className="relative inline-flex min-h-11 items-center text-[10px] font-mono tracking-widest !text-text-dim uppercase px-4 py-1.5 rounded-full transition-colors duration-300 hover:!text-text z-10 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent"
        ...
        <div className="flex-1 flex items-center justify-end gap-3">
          ...
          >
            <AnimatePresence mode="wait" initial={ false }>
              <motion.span
                ...
                initial={ shouldReduceMotion ? false : { y: 12, opacity: 0, scale: 0.8 } }
                animate={ { y: 0, opacity: 1, scale: 1 } }
                exit={ shouldReduceMotion ? { opacity: 0 } : { y: -12, opacity: 0, scale: 0.8 } }
          ...
          <a
            ...
            className="relative inline-flex min-h-11 items-center justify-center font-mono text-[9px] tracking-widest uppercase px-4 py-1.5 rounded-full transition-all duration-300 select-none active:scale-95 border border-transparent focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent
